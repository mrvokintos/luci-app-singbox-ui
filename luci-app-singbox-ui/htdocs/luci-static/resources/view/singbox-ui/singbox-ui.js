...
async function checkFile(path){
	...
	const message=String(r.stderr||r.stdout||'sing-box check failed').trim();
...
async function service(action){const r=await fs.exec(SERVICE,[action]);if((r.code|0)!==0)throw new Error('sing-box service '+action+': '+String(r.stderr||r.stdout||'failed').trim())}
...
'.sbox-row{display:grid;grid-template-columns:minmax(180px,1fr) 110px 170px auto auto;gap:12px;align-items:center;padding:12px;border:1px solid var(--border-color-medium,#e0e5ec);border-radius:8px;background:var(--background-color-medium,var(--background-color-high,#fafbfc));color:var(--text-color-high,inherit)}.sbox-row.main{border-color:var(--primary-color-medium,#77aee8);background:var(--background-color-high,#f3f8fe)}.sbox-name{font-weight:600;overflow-wrap:anywhere}.sbox-badge{font-size:11px;padding:2px 7px;border-radius:99px;background:var(--primary-color-low,#dcecff);color:var(--primary-color-high,#145da0);margin-left:7px}.muted{color:var(--text-color-medium,#667085);font-size:12px}'+
'.sbox-editor{width:100%;min-height:470px;box-sizing:border-box;padding:13px;border:1px solid var(--border-color-medium,#cfd6df);border-radius:7px;background:#111820;color:#e8edf2;font:13px/1.55 ui-monospace,Consolas,monospace;tab-size:2;resize:vertical}.sbox-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:12px;flex-wrap:wrap}.sbox-form{display:grid;grid-template-columns:1fr 240px auto;gap:10px;align-items:end}.sbox-field label{display:block;font-weight:600;margin-bottom:5px}.sbox-input{box-sizing:border-box;width:100%;padding:9px;border:1px solid var(--border-color-medium,#cfd6df);border-radius:6px;background:var(--background-color-high,#fff);color:var(--text-color-high,inherit)}.sbox-note{padding:10px 12px;border-radius:7px;background:var(--background-color-medium,var(--background-color-high,#f3f6fa));color:var(--text-color-medium,#52606d);margin:10px 0}.sbox-log{white-space:pre-wrap;overflow:auto;max-height:580px;min-height:260px;background:#111820;color:#dce4ec;border-radius:7px;padding:13px;font:12px/1.5 ui-monospace,monospace}'+
...
function init(root,initial,template){
 ...
 async function apply(name){try{const src=pathFor(name);await checkFile(src);if(src!==ACTIVE){const r=await fs.exec(HELPER,['activate',name]);if((r.code|0)!==0)throw new Error('activate config: '+String(r.stderr||r.stdout||'copy failed').trim())}await service('restart');await reload();await status();note('info',_('Config applied')+': '+name)}catch(e){note('error',_('Operation failed')+': '+e.message)}}
