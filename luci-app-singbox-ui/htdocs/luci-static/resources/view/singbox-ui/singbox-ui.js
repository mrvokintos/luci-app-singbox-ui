...
const DIR='/etc/sing-box', ACTIVE=DIR+'/config.json', TEMPLATE='/etc/singbox-ui/template.json';
...
'Active':'Активный','Back to list':'К списку','Format JSON':'Форматировать JSON','Save':'Сохранить','Share link':'Ссылка подключения',
...
async function checkFile(path){if(!await hasBin())throw new Error(_('sing-box is not installed at /usr/bin/sing-box'));const r=await fs.exec(BIN,['check','-c',path]);if((r.code|0)!==0)throw new Error(String(r.stderr||r.stdout||'sing-box check failed').trim())}
...
async function service(action){const r=await fs.exec(SERVICE,[action]);if((r.code|0)!==0)throw new Error(String(r.stderr||r.stdout||action+' failed').trim())}
...
const DEFAULT={
 ...
 route:{rules:[{action:'sniff'},{protocol:'dns',action:'hijack-dns'},{ip_is_private:true,outbound:'direct'}],final:'proxy',auto_detect_interface:true}
};
const STYLE='<style>'+
'.sbox-app{max-width:1040px;margin:auto}.sbox-head{display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:16px}.sbox-head h2{margin:0 auto 0 0}.sbox-status{display:flex;align-items:center;gap:7px;padding:5px 10px;border-radius:99px;background:#eef2f7}.sbox-dot{width:9px;height:9px;border-radius:50%;background:#9aa4b2}.running .sbox-dot{background:#1fa971;box-shadow:0 0 0 3px #1fa97122}'+
...
'.sbox-card{background:var(--background-color-high,#fff);border:1px solid #d9dfe7;border-radius:10px;padding:16px;margin-bottom:14px}.sbox-toolbar{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:12px}.sbox-toolbar .hint{margin-right:auto;color:#667085}.sbox-list{display:flex;flex-direction:column;gap:9px}'+
'.sbox-row{display:grid;grid-template-columns:minmax(180px,1fr) 110px 170px auto auto;gap:12px;align-items:center;padding:12px;border:1px solid #e0e5ec;border-radius:8px;background:#fafbfc}.sbox-row.main{border-color:#77aee8;background:#f3f8fe}.sbox-name{font-weight:600;overflow-wrap:anywhere}.sbox-badge{font-size:11px;padding:2px 7px;border-radius:99px;background:#dcecff;color:#145da0;margin-left:7px}.muted{color:#667085;font-size:12px}'+
'.sbox-editor{width:100%;min-height:470px;box-sizing:border-box;padding:13px;border:1px solid #cfd6df;border-radius:7px;background:#111820;color:#e8edf2;font:13px/1.55 ui-monospace,Consolas,monospace;tab-size:2;resize:vertical}.sbox-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:12px;flex-wrap:wrap}.sbox-form{display:grid;grid-template-columns:1fr 240px auto;gap:10px;align-items:end}.sbox-field label{display:block;font-weight:600;margin-bottom:5px}.sbox-input{box-sizing:border-box;width:100%;padding:9px;border:1px solid #cfd6df;border-radius:6px;background:var(--background-color-high,#fff);color:inherit}.sbox-note{padding:10px 12px;border-radius:7px;background:#f3f6fa;color:#52606d;margin:10px 0}.sbox-log{white-space:pre-wrap;overflow:auto;max-height:580px;min-height:260px;background:#111820;color:#dce4ec;border-radius:7px;padding:13px;font:12px/1.5 ui-monospace,monospace}'+
...
function html(){
 return STYLE+'<div class="sbox-head"><h2>'+h(_('Configuration manager'))+'</h2><span id="status" class="sbox-status"><span class="sbox-dot"></span><span>…</span></span>'+
...
function init(root,initial,template){
 let configs=initial,edited=null,saved='',list=root.querySelector('#list'),editor=root.querySelector('#editor'),tpl=root.querySelector('#templateEditor');
 tpl.value=template;
 ...
 async function apply(name){try{const src=pathFor(name);await checkFile(src);if(src!==ACTIVE){const r=await fs.exec(HELPER,['activate',name]);if((r.code|0)!==0)throw new Error(r.stderr||r.stdout||'copy failed')}await service('restart');await reload();await status();note('info',_('Config applied')+': '+name)}catch(e){note('error',_('Operation failed')+': '+e.message)}}
