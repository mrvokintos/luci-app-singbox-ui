...
const DIR='/etc/sing-box', ACTIVE=DIR+'/config.json', TEMPLATE='/etc/singbox-ui/template.json', ACTIVE_META='/etc/singbox-ui/active-config', LOG_MARKER='/etc/singbox-ui/log-clear-marker';
const BIN='/usr/bin/sing-box', SERVICE='/etc/init.d/sing-box', HELPER='/usr/libexec/singbox-ui-helper';
const UI_VERSION='3.1.1';
const RU={
...
const note=(type,msg)=>{const level=type==='error'?'error':'info',body=E('p',{},String(msg)),timeout=type==='error'?10000:5000;if(typeof ui.addTimeLimitedNotification==='function')return ui.addTimeLimitedNotification(null,body,timeout,level);const el=ui.addNotification(null,body,level);setTimeout(()=>{if(el&&el.parentNode){el.classList.add('fade-out');setTimeout(()=>el.parentNode&&el.parentNode.removeChild(el),500)}},timeout);return el};
...
'@media(max-width:800px){.sbox-row{grid-template-columns:1fr auto auto}.sbox-row .meta{display:none}.sbox-form{grid-template-columns:1fr}.sbox-form>button{width:100%}}@media(max-width:600px){.sbox-app{width:100%}.sbox-head h2{flex:1 0 100%;font-size:24px}.sbox-version,.sbox-binary-version,.sbox-status{font-size:11px;padding:5px 7px}.sbox-head>button{flex:1 1 30%;min-width:0;padding-left:5px;padding-right:5px;font-size:12px}.sbox-card{padding:10px}.sbox-tabs{gap:0}.sbox-tab{padding:9px 11px;white-space:nowrap}.sbox-toolbar{align-items:stretch}.sbox-toolbar .hint{flex:1 0 100%;margin-right:0}.sbox-toolbar button{flex:1 1 auto}.sbox-log{padding:9px;font-size:11px}}@media(max-width:520px){.sbox-row{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;padding:9px}.sbox-name{grid-column:1/-1}.sbox-row button{width:100%;min-width:0;padding-left:5px;padding-right:5px;font-size:12px}}</style>';
function html(){
 return STYLE+'<div class="sbox-head"><h2>'+h(_('Configuration manager'))+'</h2><span class="sbox-version">UI '+UI_VERSION+'</span><span id="singboxVersion" class="sbox-binary-version">sing-box …</span><span id="status" class="sbox-status"><span class="sbox-dot"></span><span>…</span></span>'+
...
function init(root,initial,template,initialActive,initialLogMarker){
 let configs=initial,activeName=initialActive,edited=null,saved='',logMarker=initialLogMarker,list=root.querySelector('#list'),editor=root.querySelector('#editor'),tpl=root.querySelector('#templateEditor');
 tpl.value=template;
 ...
 async function binaryVersion(){const e=root.querySelector('#singboxVersion');try{if(!await hasBin())throw new Error('missing');const r=await fs.exec(BIN,['version']),line=cleanError(r.stdout||r.stderr).split('\n')[0],m=line.match(/(?:sing-box\s+version\s+)?v?([0-9][\w.+-]*)/i);e.textContent=m?'sing-box '+m[1]:'sing-box —'}catch(_e){e.textContent='sing-box —'}}
 function render(){
  if(!configs.length){list.innerHTML='<div class="sbox-note">'+h(_('No JSON configs found'))+'</div>';return}
  const visible=activeName&&activeName!=='config.json'?configs.filter(x=>x.name!=='config.json'):configs;
  list.innerHTML=visible.map(x=>'<div class="sbox-row '+(x.name===activeName?'main':'')+'"><div class="sbox-name">'+h(x.name)+(x.name===activeName?'<span class="sbox-badge">'+h(_('Active'))+'</span>':'')+'</div><div class="muted meta">'+h(size(x.size))+'</div><div class="muted meta">'+h(date(x.mtime))+'</div><button class="cbi-button cbi-button-apply" data-apply="'+h(x.name)+'">'+h(_('Apply'))+'</button><button class="cbi-button" data-edit="'+h(x.name)+'">'+h(_('Edit'))+'</button></div>').join('');
 ...
 async function logState(){const b=root.querySelector('#toggleLogs');try{const cfg=JSON.parse(await read(ACTIVE));const off=cfg.log&&cfg.log.disabled===true;b.dataset.disabled=off?'1':'0';b.textContent=off?_('Enable logs'):_('Disable logs');b.classList.toggle('cbi-button-negative',!off);b.classList.toggle('cbi-button-positive',off)}catch(e){b.disabled=true;b.textContent=_('Unavailable')}}
 async function toggleLogs(){const cfg=JSON.parse(await read(ACTIVE));cfg.log=cfg.log&&typeof cfg.log==='object'&&!Array.isArray(cfg.log)?cfg.log:{};const off=cfg.log.disabled===true;cfg.log.disabled=!off;const text=JSON.stringify(cfg,null,2)+'\n';await checkText(text);await write(ACTIVE,text);await service('restart');await logState();await status();note('info',off?_('Logs enabled'):_('Logs disabled'))}
 async function logText(){const r=await fs.exec('/sbin/logread',['-e','sing-box']);return String(r.stdout||'').trim()}
 async function logs(){const el=root.querySelector('#log');try{const raw=await logText(),at=logMarker?raw.lastIndexOf(logMarker):-1,shown=at>=0?raw.slice(at+logMarker.length).trim():raw;el.textContent=shown||_('Nothing to show yet');el.scrollTop=el.scrollHeight;root.querySelector('#logTime').textContent=_('Updated')+': '+new Date().toLocaleTimeString()}catch(e){el.textContent=e.message}}
 ...
 root.querySelector('#toggleLogs').onclick=async e=>{const b=e.currentTarget;b.disabled=true;try{await toggleLogs()}catch(err){note('error',_('Operation failed')+': '+cleanError(err.message))}finally{b.disabled=false;await logState()}};
 root.querySelector('#clearLog').onclick=e=>busy(e.currentTarget,async()=>{try{await clearLogs()}catch(err){note('error',_('Operation failed')+': '+cleanError(err.message))}});
 root.querySelector('#refreshLog').onclick=e=>busy(e.currentTarget,logs);render();status();binaryVersion();logState();
}
return view.extend({handleSave:null,handleSaveApply:null,handleReset:null,async render(){const configs=await listConfigs();let template=await read(TEMPLATE);if(!template.trim())template=JSON.stringify(DEFAULT,null,2)+'\n';let activeName=(await read(ACTIVE_META,'config.json')).trim();if(!configs.some(x=>x.name===activeName))activeName=configs.some(x=>x.name==='config.json')?'config.json':'';const logMarker=(await read(LOG_MARKER)).trim();const root=E('div',{class:'sbox-app'});root.innerHTML=html();setTimeout(()=>init(root,configs,template,activeName,logMarker),0);return root}});
