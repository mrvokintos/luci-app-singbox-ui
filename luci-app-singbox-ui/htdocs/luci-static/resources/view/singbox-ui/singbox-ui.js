...
function html(){
 return STYLE+'<div class="sbox-head"><h2>'+h(_('Configuration manager'))+'<small class="sbox-version">v'+UI_VERSION+'</small></h2><span id="singboxVersion" class="sbox-binary-version">sing-box …</span><span id="status" class="sbox-status"><span class="sbox-dot"></span><span>…</span></span>'+
...
function init(root,initial,template,initialActive){
 let configs=initial,activeName=initialActive,edited=null,saved='',logMarker='',list=root.querySelector('#list'),editor=root.querySelector('#editor'),tpl=root.querySelector('#templateEditor');
 tpl.value=template;
 ...
 async function clearLogs(){const raw=await logText(),lines=raw.split('\n').filter(Boolean);logMarker=lines.length?lines[lines.length-1]:'';root.querySelector('#log').textContent=_('Nothing to show yet');root.querySelector('#logTime').textContent=''}
